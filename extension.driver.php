...
//			if(isset($div))$ul->appendChild(new XMLElement('il', 'Database: Grab data and schamas for workspace'));
			...
			// Sql Authors
			$sql_data_authors = $dump->export('tbl_authors', MySQLDump::DATA_ONLY);
			$sql_data_authors =  str_replace('`' . Administration::instance()->Configuration->get('tbl_prefix', 'database'), '`tbl_', $sql_data_authors);
...
//				if(isset($div))$ul->appendChild(new XMLElement('il', 'ZIP: colleting install scripts'));
				...
				$archive->addFromString('META-INF/authors.sql', $sql_data_authors);
...
*/			
		...
		public function savePreferences($context){
			...
			$context['settings']['deployer'] = array(
								  'auto-maintence' => (isset($conf['auto-maintence']) ? $conf['auto-maintence'] : 'yes'),
								  'storage-url' => (isset($conf['storage-url']) ? $conf['storage-url'] : DOCROOT . '/backup'),
								  'include-authors' =>  (isset($conf['include-authors']) ? $conf['include-authors'] : 'no'),
								  'deploy-url' => (isset($conf['deploy-url']) ? $conf['deply-url'] : 'admin')
								);
		...
		public function appendPreferences($context){
			...
			$label = Widget::Label(__('Deploy Path'));
			...
			$group->appendChild(new XMLElement('p', __('This is the path where extract deployer Ensemble'), array('class' => 'help')));

			$label = Widget::Label(__('Storage Path'));
			...
			$group->appendChild($label);
			$group->appendChild(new XMLElement('p', __('This is the path where deployer Ensemble are stored ('. DOCROOT . '/backup)'), array('class' => 'help')));
...
/*
			$label = Widget::Label();
			$input = Widget::Input('settings[deployer][include-authors]', 'yes', 'checkbox');
			if(Administration::instance()->Configuration->get('include-authors', 'deployer') == 'yes') $input->setAttribute('checked', 'checked');
			$label->setValue($input->generate() . ' Include authors in database deployer');
			$group->appendChild($label);
...
*/
			$label = Widget::Label();
			$input = Widget::Select('settings[deployer][include-authors]', array(
					array('yes',Administration::instance()->Configuration->get('include-authors', 'deployer') == 'yes',__('Always'))
					,array('no',Administration::instance()->Configuration->get('include-authors', 'deployer') == 'no',__('Never'))
					,array('admin',Administration::instance()->Configuration->get('include-authors', 'deployer') == 'admin',__('Only Admin (first author)'))
				) );
			$label->setValue(__(' Import/Sync authors in deploy').$input->generate());
			$group->appendChild($label);
			$group->appendChild(new XMLElement('p', __('After the deploy if is an installed enviroment also Authors table data is sync'), array('class' => 'help')));


			$label = Widget::Label();
			$input = Widget::Select('settings[deployer][auto-maintence]', array(
					array('yes',Administration::instance()->Configuration->get('auto-maintence', 'deployer') == 'yes',__('Always'))
					,array('no',Administration::instance()->Configuration->get('auto-maintence', 'deployer') == 'no',__('Never'))
					,array('auto',Administration::instance()->Configuration->get('auto-maintence', 'deployer') == 'auto',__('Only on Hard Deploy'))
				) );
			$label->setValue(__(' Automatc Maintence mode on deploy').$input->generate());
			$group->appendChild($label);
			$group->appendChild(new XMLElement('p', __('Puts the production environment in "Maintence Mode" automatically during the deployment operation.'), array('class' => 'help')));
...
//					$dpy_status = ($settings['maintenance_mode']['enabled'] == 'no')?'Production':'Maintence';
					...
					while (($sql = fgets($handle, 4096)) !== false) {
						 $query.=$sql;
						 if(strpos($sql,';') !== false && trim($query) != ''){
							if(preg_match('/.*TABLE `(.*)`.*/', $query,$matches) != 0){
								...
								if(strcmp(trim($sql_schema),trim($query)) != 0){
									...
									if($deployType=='SOFT'){
										...
										if(Administration::instance()->Configuration->get('auto-maintence', 'deployer') == 'auto')$oldMaintenceStatus = self::productionMaintence(true,$deploy_log);
...
//						 	  $deploy_log->writeToLog("EXEC: ".$query);
						 	 $mysql->import($query);
						 	 $query='';
						 }
					}        
					...
					if(Administration::instance()->Configuration->get('include-authors', 'deployer') != 'no'){
						$query='';
						$deploy_log->writeToLog("INFO: Importing authors data..START", true);
						$handle = fopen($this->getDeployUrl().'/META-INF/authors.sql', "r");
						while (($sql = fgets($handle, 4096)) !== false) {
							if(Administration::instance()->Configuration->get('include-authors', 'deployer') == 'admin'){
								if(strpos($sql,"VALUES (1,") !==false)$query.=$sql;
							} else {
								$query.=$sql;
							}
							if( strpos($sql,";\r\n") !== false  && trim($query) != ''){
								 $query = preg_replace('/INSERT/i', 'REPLACE', $query);
//								 $deploy_log->writeToLog("EXEC: ".$sql."\n".$query."\n\n\n");
								 $mysql->import($query);
								 $query='';
							 }
						}        
						fclose($handle);						
						$deploy_log->writeToLog("INFO: Importing authors data..END", true);
					}
			...
			if(isset($oldMaintenceStatus))self::productionMaintence($oldMaintenceStatus,$deploy_log);
		...
		public function productionMaintence($status,$deploy_log=NULL){
				if(isset($deploy_log))$deploy_log->writeToLog("INFO: Request a product maintence switch", true);
				if(file_exists($this->getDeployUrl().'/manifest/config.php')){
					require($this->getDeployUrl().'/manifest/config.php');
					...
					fclose($handle);
					if(isset($deploy_log))$deploy_log->writeToLog("INFO: Switch made from ".(($oldStatus=='yes')?'Maintence':'Production')." to ".($status?'Maintence':'Production'), true);
					return ($oldStatus='yes')?true:false;
				} else {
				if(isset($deploy_log))$deploy_log->writeToLog("INFO: Target environment not installed! Ignore switch!", true);
				}
		}
